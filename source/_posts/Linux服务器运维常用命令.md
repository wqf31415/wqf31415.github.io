...
`cal` ，日历（calendar）命令用于显示日历，不带参数时显示本月日历，日期当天回反白显示。可指定参数 月 和 年，以显示指定年月的日历，如 `cal 10 2022` 显示 2022 年 10 月的日历。
...
`fdisk` 是一个磁盘分区管理工具，可用于对磁盘进行分区、删除分区、查看磁盘分区信息。

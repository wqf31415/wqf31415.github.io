...
正如初始界面中提示的，我们可以输入命令查看爆出或退出：
指令|作用
----|----
`:help iccf` <kbd>回车</kbd> | 查看说明
`:q` <kbd>回车</kbd> | **退出(Quit)** 
`:help` <kbd>回车</kbd> | 查看在线帮助
`:help version8` <kbd>回车</kbd> | 查看版本信息
...
编辑完成后，按 <kbd>Esc</kbd> 键回到 normal 模式，输入 `:wq` <kbd>回车</kbd> 保存文件，也可以输入 `:wq` <kbd>回车</kbd> 保存并退出。 

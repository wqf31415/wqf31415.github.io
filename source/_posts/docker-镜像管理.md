...
![](/images/post/docker/docker_whale_2.webp)
...
删除悬空镜像（即仓库名和标签都是 <none>的废弃镜像，通常是由于构建失败或重新构建产生的残留）：
...
导出镜像是指将镜像连同所有的元数据、标签、历史记录打包成一个 `.tar` 文件。

#### 导出镜像

```bash
docker save 镜像名:版本 -o 镜像文件名.tar
```

> 参数说明：
> - `镜像名:版本` 表示要导出的镜像的名称和版本号，多个镜像 ID 之间空格隔开。
> - `镜像文件名.tar` 表示导出的镜像文件名，不指定时默认导出为 `镜像名.tar`。

#### 导出并压缩

```bash
docker save 镜像名:版本 | gzip > 镜像文件名.tar.gz
````

> 参数说明：
> - `镜像名:版本` 表示要导出的镜像的名称和版本号。
> - `镜像文件名.tar.gz` 表示导出的镜像压缩文件名。
...
```bash
docker save 镜像名1:版本1 镜像名2:版本2 -o 镜像文件名.tar
```

> 参数说明：
> - `镜像名1:版本1`、`镜像名2:版本2` 表示要导出的镜像的名称和版本号，多个镜像名称和版本号之间用空格隔开。
> - `镜像文件名.tar` 表示导出的镜像文件名。
...
> 参数说明：
> - `镜像文件名.tar` 表示要导入的镜像文件名，不指定时默认导入为 `镜像名.tar`。
...
```bash
docker load -i 镜像文件名.tar.gz
```

> 参数说明：
> - `镜像文件名.tar.gz` 表示要导入的镜像压缩文件名，不指定时默认导入为 `镜像名.tar.gz`。

...
![git](/images/post/git/git_logo.webp "git")
...
![](/images/post/git/git_workflow.webp "git 工作流")
...
![](/images/post/git/git_gitBash_0.webp "git bash here")



git bash 命令行如下图所示：
![](/images/post/git/git_gitBash_cli.webp "Git Bash 命令行")

我们可以输入 `git --version` 来查看git版本
![](/images/post/git/git_gitBash_gitVersion.webp "查看git版本")
...
![](/images/post/git/git_config_list.webp "查看git设置")

也可以输入 `git config user.name` 单独查看用户名
![](/images/post/git/git_look_username.webp "查看git用户名")
...
我们可以在一个空的文件夹里初始化仓库，也可以在一个用文件的文件夹里初始化，只需要我们在要初始化的文件空白区域右击打开 git bash 工具，执行 `git init` 命令即可，执行完成后将会在该目录下创建一个名叫 `.git` 的隐藏文件夹，这里面存放的是git仓库的所有信息文件。
 ![](/images/post/git/git_init.webp "初始化git仓库")
...
![](/images/post/git/git_status_1.webp "git 仓库状态") 
...
##### 添加单个文件
 ...
 ![](/images/post/git/git_add_single_file.webp "添加单个文件")
...
##### 添加同一类型的文件
 ...
 ![](/images/post/git/git_add_one_type_files.webp "添加一个类型的文件")
...
`git add .` 直接将目录下所有文件添加到git仓库；
 ![](/images/post/git/git_add_all_files.webp "添加所有文件")

 #### 删除已添加的文件

 如果不小心把不需要跟踪的文件（如1.txt）添加到了仓库，可以使用 `git rm --cached 1.txt` 将文件从已添加文件中删除：
 ![](/images/post/git/git_rm_single_file.webp "删除已添加文件")

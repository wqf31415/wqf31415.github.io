...
| 光猫            |                   | 网络运营商提供 |                                         |
| 交换机          | 八孔千兆          | 购于京东       |                                         |
| 路由器          |                   | 网络运营商提供 | 已于房间之间距离比较远，安装了3个路由器 |
| 笔记本电脑      | Windows 11系统    | -              |                                         |
| 网线            | 超六类，1米       | 购于京东       |                                         |
...
将 TF 卡连接到电脑，打开 balenaEtcher 软件，选择下载的镜像文件，烧录的目标选择插入的 TF 卡，点击开始烧录，等待系统烧录完成，约 1 分钟即可烧录完成。

> balenaEtcher是一款用于烧录操作系统到SD卡或USB设备的软件，下载地址是: <https://etcher.balena.io/>
...
给 R3S 上电，电源指示灯先快速闪动，最后常量，说明系统启动完成。此时看 wan、lan指示灯亮起。

电脑连接到路由器或交换机，等待电脑获取到 IP ，查看电脑 IP，发现是 `192.168.1.*` 的网段，打开浏览器访问 R3S 的管理页面：<http://192.168.1.1> 。



系统默认用户是 root，没有密码，输入用户名后直接登录。
...
进入管理页面的 `网络` --> `接口` 页面，看到 lan 口的 IPv4 地址是 `192.168.1.1/24` ，和 wan 口 IP 是同一个段，需要修改。点击编辑，将 IPv4地址修改为：`192.168.2.1` ，IPv4 广播地址自动更新为：`192.168.2.255` ，点击保存。
...
安装程序的方式有两种，一是通过 ssh 访问软路由，如 `ssh root@192.168.2.1` ，在命令行窗口通过包管理工具 opkg 进行安装，如 `opkg install luci-i18n-passwall-zh-cn` ；二是在软路由管理界面的 `系统` --> `软件包` 界面搜索软件包，点击安装。
...
|  2   |     luci-theme-argon     |    主题    | 建议，比原始主题好看 |
|  3   | luci-i18n-passwall-zh-cn |  代理工具  |         建议         |
...
安装上网代理工具 `luci-i18n-passwall-zh-cn` 后，重新登录软路由管理页面，进入 `服务` --> `PassWall` 页面，在 `节点订阅` 选项卡中点击 `添加` ，填写机场备注、订阅地址，其他选项可以保持默认，点击保存并应用。



### 总结

有了软路由后，再配置好上网代理，家里所有设备都可以科学上网了，而且如果要换网络运营商，只需要更换光猫就可以了，软路由以及后面的交换机、路由器都不需要调整，非常方便。
